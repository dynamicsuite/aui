...
Vue.component("aui-badge",{template:'<span class="aui badge" :class="style_class"><slot>{{text}}</slot></span>',props:{type:{type:String,default:"primary",validator:e=>-1!==["none","primary","secondary","success","warning","failure"].indexOf(e)},text:{type:String|Number}},computed:{style_class(){return{[this.type]:"none"!==this.type}}}});
...
Vue.component("aui-table",{template:'<div class="aui table"><div class="options"><i class="fas fa-columns" title="Select Columns To View" @click="showColumnsModal"/></div><aui-modal :show.sync="show_columns_modal" title="Select Columns To View"><template #content><aui-checkbox v-for="(selected, column) in columns_modal_model" :key="\'column\' + column" :label="columnName(column)" :checked="selected" v-model="columns_modal_model[column]"/><aui-alert v-if="columns_modal_feedback" type="failure" :text="columns_modal_feedback"/></template><template #left><aui-button type="secondary" text="Cancel" @click="hideColumnsModal"/></template><template #right><div class="btn-group"><aui-button text="Apply" @click="applyColumns"/><aui-button-drop :dropdown="{reset: \'Reset to Default\'}" relative_to=".modal" @reset="resetColumns"/></div></template></aui-modal><table :class="table_classes"><thead><tr><th v-for="(column, id) in columns" :key="\'header\' + id" ref="header" @mousedown.self="runSort(column)"><template>{{columnName(column)}}</template><i v-if="sortable" :class="sortIcon(column)"/><div @mousedown="handleResizeColumn($event, id)"/></th></tr></thead><tbody><tr v-for="(row, id) in filtered_table" :key="\'row\' + id" @click="rowInteraction(id)"><td v-for="(column_value, column) in row" :key="\'column\' + column + id">{{columnValue(column, column_value)}}</td></tr></tbody></table></div>',props:{table:{type:Array,default:()=>[]},interactive:{type:Boolean,default:!0},interactive_column:{type:null|String,default:null},default_columns:{type:Array,default:()=>[]},column_names:{type:Object,default:()=>({})},column_format:{type:Object,default:()=>({})},sortable:{type:Boolean,default:!1},sort:{type:Object,default:()=>({})},sort_icon_none:{type:String,default:"fas fa-sort"},sort_icon_asc:{type:String,default:"fas fa-sort-amount-down-alt"},sort_icon_desc:{type:String,default:"fas fa-sort-amount-down"},local_storage_key:{type:null|String,default:null},calling:{type:Boolean,default:!1}},data:()=>({columns:[],show_columns_modal:!1,columns_modal_model:{},columns_modal_feedback:null}),computed:{table_classes(){return{interactive:this.interactive,sortable:this.sortable}},available_columns(){return"object"!=typeof this.table[0]?[]:Object.keys(this.table[0])},filtered_table(){const t=[];for(const e of this.table)t.push(Object.keys(e).filter(t=>this.columns.includes(t)).reduce((t,o)=>(t[o]=e[o],t),{}));return t}},methods:{columnName(t){return this.column_names.hasOwnProperty(t)?this.column_names[t]:t},columnValue(t,e){return this.column_format.hasOwnProperty(t)?this.column_format[t](e):e},sortIcon(t){return{[this.sort_icon_none]:!this.sort.hasOwnProperty(t),[this.sort_icon_asc]:this.sort.hasOwnProperty(t)&&"ASC"===this.sort[t],[this.sort_icon_desc]:this.sort.hasOwnProperty(t)&&"DESC"===this.sort[t]}},runSort(t){if(!this.sortable||this.calling)return;let e=JSON.parse(JSON.stringify(this.sort));"object"!=typeof e&&(e={}),e.hasOwnProperty(t)?"ASC"===e[t]?e[t]="DESC":delete e[t]:e[t]="ASC",this.$emit("update:sort",e)},rowInteraction(t){this.interactive&&!this.calling&&this.interactive_column&&this.filtered_table[t].hasOwnProperty(this.interactive_column)&&this.$emit("row-interaction",this.filtered_table[t][this.interactive_column])},handleResizeColumn(t,e){let o=!0;const l=t.pageX,s=parseInt(window.getComputedStyle(this.$refs.header[e]).width);document.addEventListener("mousemove",t=>{if(o){const o=s+(t.pageX-l);this.$refs.header[e].style.width=`${o}px`}}),document.addEventListener("mouseup",()=>{o=!1})},resetColumns(){this.default_columns.length?this.columns=this.default_columns:this.columns=JSON.parse(JSON.stringify(this.available_columns)),this.show_columns_modal=!1,this.local_storage_key&&localStorage.removeItem(this.local_storage_key)},showColumnsModal(){if(this.calling)return;const t={};for(const e of this.available_columns)t[e]=-1!==this.columns.indexOf(e);this.$set(this,"columns_modal_model",t),this.columns_modal_feedback=!1,this.show_columns_modal=!0},hideColumnsModal(){this.show_columns_modal=!1},applyColumns(){const t=[];for(const e in this.columns_modal_model)this.columns_modal_model[e]&&t.push(e);t.length?(this.$set(this,"columns",t),this.saveToLocalStorage(),this.hideColumnsModal()):this.columns_modal_feedback="Must select at least 1 column"},saveToLocalStorage(){this.local_storage_key&&localStorage.setItem(this.local_storage_key,JSON.stringify(this.columns))},getFromLocalStorage(){if(!this.local_storage_key)return!1;const t=localStorage.getItem(this.local_storage_key);return!!t&&JSON.parse(t)}},mounted(){const t=this.getFromLocalStorage();if(t){let e=!0;for(const o of t)-1===this.available_columns.indexOf(o)&&(e=!1);e?this.$set(this,"columns",t):this.resetColumns()}else this.resetColumns()}});
Vue.component("aui-tabs",{template:'<div class="aui tabs"><span v-if="!collapsed" v-for="(name, tab) in tabs" v-html="name" class="tab" :class="tabClass(tab)" @click="handleClick(tab)"/><aui-button-drop v-if="collapsed" :dropdown="tabs" :text="tabs[tab]" @menu-click="handleClick" menu_align="left"/></div>',props:{tabs:{type:Object,required:!0},tab:{type:String|Number,required:!0},disabled:{type:Boolean,default:!1}},data:()=>({width:0,collapsed:!1}),methods:{handleClick(e){this.disabled||(this.$emit("update:tab",e),this.$emit("change",e))},tabClass(e){return{disabled:this.disabled,active:e===this.tab}},checkIfCollapsed(){window.requestAnimationFrame(()=>{this.collapsed=this.$el.clientWidth<=this.width||this.width+40>=this.$el.parentElement.clientWidth})}},mounted(){window.addEventListener("resize",this.checkIfCollapsed),this.$nextTick(()=>{for(let e=0;e<this.$el.childNodes.length;e++)"number"==typeof this.$el.childNodes[e].offsetWidth&&(this.width+=this.$el.childNodes[e].offsetWidth+1);this.width+=32,this.checkIfCollapsed()})},beforeDestroy(){window.removeEventListener("resize",this.checkIfCollapsed)}});
Vue.component("aui-textarea",{template:'<aui-form-control v-bind="properties"><textarea :value="value" :placeholder="placeholder" :autocomplete="autocomplete" :disabled="disabled" :rows="rows" @focus="$emit(\'focus\', $event.target)" @blur="$emit(\'blur\', $event.target)" @input="$emit(\'input\', $event.target.value)" @keydown="$emit(\'keydown\', $event)"/></aui-form-control>',props:{label:{type:null|String,default:null},leading_cap:{type:null|String,default:null},trailing_cap:{type:null|String,default:null},value:{type:String|Number|Boolean|null,default:null},autocomplete:{type:null|String,default:null},placeholder:{type:null|String,default:null},disabled:{type:Boolean,default:!1},rows:{type:String|Number|null,default:null},success:{type:null|String,default:null},failure:{type:null|String,default:null},subtext:{type:null|String,default:null}},computed:{properties(){return{parent:"textarea",label:this.$slots.default?this.$slots.default:this.label,leading_cap:this.leading_cap,trailing_cap:this.trailing_cap,success:this.success,failure:this.failure,subtext:this.subtext}}}});

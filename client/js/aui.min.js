...
Vue.component("aui-number",{template:'<span class="aui number" :class="negative_class">{{pretty_value}}</span>',props:{is_currency:{type:Boolean,default:!1},currency_sign:{type:String,default:"$"},decimals:{type:Number,default:0,validator:function(t){return t>=0&&parseInt(t)===t}}},computed:{negative_class(){return this.$slots.default[0].text>=0?"":"negative"},pretty_value(){let t=this.$slots.default[0].text,e=this.is_currency?this.currency_sign:"",s=t<0,a=(t+"").split(".")[0].toString(),i=(t+"").split(".")[1].toString();a=parseFloat(a.toString().replace(",","")),a=Math.abs(a).toString();let r=parseFloat(a+"."+i),l=i&&i.length<this.decimals?r.toFixed(this.decimals):r.toFixed(i.length),n=(l+"").split(".")[0].toString(),u=(l+"").split(".")[1].toString();return n=n.replace(/\B(?=(\d{3})+(?!\d))/g,","),(s?`-${e}`:`${e}`)+n+"."+u}}});

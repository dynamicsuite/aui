...
Vue.component("aui-number",{template:'<span class="aui number" :class="negative_class">{{pretty_value}}</span>',props:{is_currency:{type:Boolean,default:!1},currency_sign:{type:String,default:"$"},decimals:{type:Number,default:0,validator:t=>t>=0&&parseInt(t)===t}},computed:{negative_class(){return{negative:this.$slots.default[0].text>=0}},pretty_value(){const t=this.$slots.default[0].text,e=this.is_currency?this.currency_sign:"",s=t<0;let a=t.split(".")[0].toString();const i=t.split(".")[1].toString();a=parseFloat(a.toString().replace(",","")),a=Math.abs(a).toString();const n=parseFloat(`${a}.${i}`),l=i&&i.length<this.decimals?n.toFixed(this.decimals):n.toFixed(i.length);let r=l.split(".")[0].toString();const c=l.split(".")[1].toString();return r=r.replace(/\B(?=(\d{3})+(?!\d))/g,","),(s?`-${e}`:`${e}`)+r+"."+c}}});

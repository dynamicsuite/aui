...
Vue.component("aui-telephone",{template:'<div class="aui telephone"><aui-input class="country" v-if="international" v-model="country_code" @input="inputHandler(\'input\')" @change="inputHandler(\'change\')"></aui-input><aui-input class="phone-number" label="Phone Number" type="tel" v-model="number" @input="inputHandler(\'input\')" @change="inputHandler(\'change\')"></aui-input><aui-input class="extension" label="Ext." v-if="extension" v-model="extension_code" @input="inputHandler(\'input\')" @change="inputHandler(\'change\')"></aui-input></div>',props:{international:{type:Boolean,default:!1},extension:{type:Boolean,default:!1}},data:()=>({country_code:"+1",number:null,extension_code:null}),methods:{maskCode(){this.country_code=`+${this.country_code.match(/\d+/)[0]}`},maskNumber(){let e=this.number.toString().replace("(","").replace(")","").replace("-","").replace(" ","");e.length<3?this.number=e:e.length<6?this.number=e.toString().replace(/(\d{3})/,"($1) ").slice(0,14):(this.number=e.toString().replace(/(\d{3})(\d{1,3})/,"($1) $2-").slice(0,14),this.$forceUpdate())},emitPhone(e){let n=this.number;n&&(n=n.replace(/\D/g,"").substring(0,10),this.international&&(n=this.country_code+n),this.extension&&null!==this.extension_code&&""!==this.extension_code&&(n=n+" ext. "+this.extension_code)),this.$emit(e,n)},inputHandler(e){this.maskCode(),this.maskNumber(),this.emitPhone(e)}}});

...
Vue.component("aui-input",{template:'<div :id="id + \'-container\'" class="aui input"><label>{{label}}<span><span v-if="leading_cap" class="leading-cap" :class="input_classes" v-html="leading_cap"></span><input :id="id" :class="input_classes" :name="name" :type="type" :placeholder="placeholder" :tabindex="tabindex" :disabled="disabled" :value="value" :step="step" :min="min" :max="max" :autocomplete="autocomplete" ref="input" @keydown="$emit(\'keydown\', $event)" @change="$emit(\'change\', $event.target)" @select="$emit(\'select\', $event.target)" @focus="$emit(\'focus\', $event.target)" @blur="$emit(\'blur\', $event.target)" @input="$emit(\'input\', $event.target.value)" /><span v-if="trailing_cap" class="trailing-cap" :class="input_classes" v-html="trailing_cap"></span></span></label><div v-if="subtext_value" class="subtext" :class="subtext_classes">{{subtext_value}}</div></div>',props:{id:{type:String,required:!0},label:{type:String},leading_cap:{type:String},trailing_cap:{type:String},name:{type:String},type:{type:String,default:"text",validator:e=>-1!==["color","date","datetime-local","email","file","number","password","range","search","tel","text","time","url","week"].indexOf(e)},placeholder:{type:String},tabindex:{type:String|Number},disabled:{type:Boolean,default:!1},value:{type:String|Number|Boolean},step:{type:String|Number},min:{type:String|Number},max:{type:String|Number},autocomplete:{type:String,default:"on",validator:e=>-1!==["on","off"].indexOf(e)},success:{type:String|Boolean,default:!1},failure:{type:String|Boolean,default:!1},subtext:{type:String},no_feedback_icon:{type:Boolean,default:!1}},computed:{input_classes(){return{"border-success":this.success,"border-failure":this.failure,"no-feedback-icon":this.no_feedback_icon}},subtext_classes(){return{"text-success":this.success,"text-failure":this.failure}},subtext_value(){return this.success?this.success:this.failure?this.failure:this.subtext}},methods:{focus(){this.$refs.input.focus()},select(){this.$refs.input.select()}}});
...
Vue.component("aui-select",{template:'<div :id="id + \'-container\'" class="aui select"><label>{{label}}<select :id="id" :class="input_classes" :name="name" :disabled="disabled" :value="value" @input="$emit(\'input\', $event.target.value)" @change="$emit(\'change\', $event.target.value)"><option v-for="(option, value) in render_options" :value="value">{{option}}</option></select></label><div v-if="subtext_value" class="subtext" :class="subtext_classes">{{subtext_value}}</div></div>',props:{id:{type:String,required:!0},label:{type:String},name:{type:String},disabled:{type:Boolean,default:!1},value:{type:String|Number|Boolean},options:{type:Array|Object},success:{type:String|Boolean,default:!1},failure:{type:String|Boolean,default:!1},subtext:{type:String},us_states:{type:Boolean,default:!1}},data:()=>({states:{0:"",AL:"Alabama",AK:"Alaska",AS:"American Samoa",AZ:"Arizona",AR:"Arkansas",CA:"California",CO:"Colorado",CT:"Connecticut",DE:"Delaware",DC:"District Of Columbia",FM:"Federated States Of Micronesia",FL:"Florida",GA:"Georgia",GU:"Guam",HI:"Hawaii",ID:"Idaho",IL:"Illinois",IN:"Indiana",IA:"Iowa",KS:"Kansas",KY:"Kentucky",LA:"Louisiana",ME:"Maine",MH:"Marshall Islands",MD:"Maryland",MA:"Massachusetts",MI:"Michigan",MN:"Minnesota",MS:"Mississippi",MO:"Missouri",MT:"Montana",NE:"Nebraska",NV:"Nevada",NH:"New Hampshire",NJ:"New Jersey",NM:"New Mexico",NY:"New York",NC:"North Carolina",ND:"North Dakota",MP:"Northern Mariana Islands",OH:"Ohio",OK:"Oklahoma",OR:"Oregon",PW:"Palau",PA:"Pennsylvania",PR:"Puerto Rico",RI:"Rhode Island",SC:"South Carolina",SD:"South Dakota",TN:"Tennessee",TX:"Texas",UT:"Utah",VT:"Vermont",VI:"Virgin Islands",VA:"Virginia",WA:"Washington",WV:"West Virginia",WI:"Wisconsin",WY:"Wyoming"}}),computed:{render_options(){return this.us_states?this.states:this.options},input_classes(){return{"border-success":this.success,"border-failure":this.failure}},subtext_classes(){return{"text-success":this.success,"text-failure":this.failure}},subtext_value(){return this.success?this.success:this.failure?this.failure:this.subtext}}});

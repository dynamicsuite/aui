...
Vue.component("aui-radio",{template:'<label class="aui radio"><slot>{{label}}</slot><input type="radio" :name="group" :value="data" :disabled="disabled" @change="$emit(\'input\', $event.target.value)"><span class="bubble"></span></label>',props:{label:{type:String},group:{type:String,required:!0},disabled:{type:Boolean,default:!1},data:{type:String|Number|Boolean}}});
Vue.component("aui-select",{template:'<div :id="container_id" class="aui select"><label><span class="label-text" v-if="label">{{label}}</span><select :id="id" :class="input_classes" :name="name_computed" :disabled="disabled" :value="value" :autocomplete="autocomplete_computed" @input="$emit(\'input\', $event.target.value)" @change="$emit(\'change\', $event.target.value)"><option v-for="(option, key) in render_options" :value="key" :selected="key == value">{{option}}</option></select></label><div v-if="subtext_value" class="subtext" :class="subtext_classes">{{subtext_value}}</div></div>',props:{id:{type:String},label:{type:String},name:{type:String},disabled:{type:Boolean,default:!1},value:{type:String|Number|Boolean},options:{type:Array|Object},success:{type:String|Boolean,default:!1},failure:{type:String|Boolean,default:!1},subtext:{type:String},us_states:{type:Boolean,default:!1},no_feedback_icon:{type:Boolean,default:!1},disable_autofill:{type:Boolean,default:!0}},data:()=>({states:{0:"",AL:"Alabama",AK:"Alaska",AS:"American Samoa",AZ:"Arizona",AR:"Arkansas",CA:"California",CO:"Colorado",CT:"Connecticut",DE:"Delaware",DC:"District Of Columbia",FM:"Federated States Of Micronesia",FL:"Florida",GA:"Georgia",GU:"Guam",HI:"Hawaii",ID:"Idaho",IL:"Illinois",IN:"Indiana",IA:"Iowa",KS:"Kansas",KY:"Kentucky",LA:"Louisiana",ME:"Maine",MH:"Marshall Islands",MD:"Maryland",MA:"Massachusetts",MI:"Michigan",MN:"Minnesota",MS:"Mississippi",MO:"Missouri",MT:"Montana",NE:"Nebraska",NV:"Nevada",NH:"New Hampshire",NJ:"New Jersey",NM:"New Mexico",NY:"New York",NC:"North Carolina",ND:"North Dakota",MP:"Northern Mariana Islands",OH:"Ohio",OK:"Oklahoma",OR:"Oregon",PW:"Palau",PA:"Pennsylvania",PR:"Puerto Rico",RI:"Rhode Island",SC:"South Carolina",SD:"South Dakota",TN:"Tennessee",TX:"Texas",UT:"Utah",VT:"Vermont",VI:"Virgin Islands",VA:"Virginia",WA:"Washington",WV:"West Virginia",WI:"Wisconsin",WY:"Wyoming"}}),computed:{render_options(){return this.us_states?this.states:this.options},input_classes(){return{"border-success":this.success,"border-failure":this.failure,"no-feedback-icon":this.no_feedback_icon}},subtext_classes(){return{"text-success":this.success,"text-failure":this.failure}},subtext_value(){return"string"==typeof this.success?this.success:"string"==typeof this.failure?this.failure:this.subtext},container_id(){return this.id?this.id+"-container":""},autocomplete_computed(){return this.disable_autofill||"off"===this.autocomplete?"new-password":"on"},name_computed(){return this.disable_autofill?"autofill-disabled-"+Math.random().toString(36).substring(2,15)+Math.random().toString(36).substring(2,15):this.name}},watch:{failure(){clearTimeout(window.aui_failure_timeout),window.aui_failure_timeout=setTimeout(()=>{let e=document.getElementById("ds-view");if(!e)return;let t=document.querySelectorAll(".aui .border-failure")[0];if(t){let a=t.getBoundingClientRect(),i=a.top-a.height-100;e.scrollBy({top:i,behavior:"smooth"})}},99)}}});
Vue.component("aui-select-assignment",{template:'<div class="aui select-assignment"><div class="aui select assigned"><label><span class="label-text">{{assigned_label}}</span><select multiple :disabled="disabled" v-model="assigned_selected"><option v-for="(label, value) in assigned" :value="value">{{label}}</option></select></label><aui-button :text="\'Move to &quot;\' + unassigned_label + \'&quot; list\'" type="secondary" :disabled="disabled" @click="move(\'assigned\')" /></div><div class="aui select unassigned"><label><span class="label-text">{{unassigned_label}}</span><select multiple :disabled="disabled" v-model="unassigned_selected"><option v-for="(label, value) in unassigned" :value="value">{{label}}</option></select></label><aui-button :text="\'Move to &quot;\' + assigned_label + \'&quot; list\'" type="secondary" :disabled="disabled" @click="move(\'unassigned\')" /></div></div>',props:{assigned:{type:null|Object,required:!0},assigned_label:{type:String,default:"Assigned"},unassigned:{type:null|Object,required:!0},unassigned_label:{type:String,default:"Unassigned"},disabled:{type:Boolean,default:!1}},data:()=>({assigned_selected:[],unassigned_selected:[]}),methods:{move(e){const s=Object.assign({},this.assigned),l=Object.assign({},this.unassigned);for(const a of this[`${e}_selected`])"assigned"===e?(l[a]=s[a],delete s[a]):(s[a]=l[a],delete l[a]);this.$emit("update:assigned",s),this.$emit("update:unassigned",l),this[`${e}_selected`]=[]}}});
...
Vue.component("aui-telephone",{template:'<div class="aui telephone"><aui-input class="country" label="Intl." v-if="international" v-model="country_code" :disabled="disabled" @input="inputHandler(\'input\')" @change="inputHandler(\'change\')"></aui-input><aui-input class="phone-number" :label="label" type="tel" v-model="number" :disabled="disabled" @input="inputHandler(\'input\')" @change="inputHandler(\'change\')"></aui-input><aui-input class="extension" label="Ext." v-if="extension" v-model="extension_code" :disabled="disabled" @input="inputHandler(\'input\')" @change="inputHandler(\'change\')"></aui-input></div>',props:{international:{type:Boolean,default:!1},extension:{type:Boolean,default:!1},label:{type:String,default:"Phone Number"},disabled:{type:Boolean,default:!1}},data:()=>({country_code:"+1",number:null,extension_code:null}),methods:{maskCode(){this.country_code=`+${this.country_code.match(/\d+/)[0]}`},maskNumber(){let e=this.number.toString().replace("(","").replace(")","").replace("-","").replace(" ","");e.length<3?this.number=e:e.length<6?this.number=e.toString().replace(/(\d{3})/,"($1) ").slice(0,14):(this.number=e.toString().replace(/(\d{3})(\d{1,3})/,"($1) $2-").slice(0,14),this.$forceUpdate())},emitPhone(e){let n=this.number;n&&(n=n.replace(/\D/g,"").substring(0,10),this.international&&(n=this.country_code+n),this.extension&&null!==this.extension_code&&""!==this.extension_code&&(n=n+" ext. "+this.extension_code)),this.$emit(e,n)},inputHandler(e){this.maskCode(),this.maskNumber(),this.emitPhone(e)}}});
...
Vue.component("aui-wysiwyg",{template:'<div class="aui wysiwyg"><label>{{label}}</label><div><div v-if="disabled" class="overlay"></div><div ref="editor" :class="editor_classes"></div></div><div v-if="subtext_value" :class="subtext_classes">{{subtext_value}}</div></div>',props:{value:{type:String},label:{type:String},actions:{type:Array,default:()=>["bold","italic","underline","heading1","ulist","olist"]},default_paragraph_separator:{type:String,default:"p"},style_with_css:{type:Boolean,default:!0},classes:{type:Object,default:()=>({actionbar:"pell-actionbar",button:"pell-btn",content:"pell-content",selected:"pell-button-selected"})},disabled:{type:Boolean,default:!1},success:{type:String|Boolean,default:!1},failure:{type:String|Boolean,default:!1},subtext:{type:String}},data:()=>({editor:null}),computed:{editor_classes(){return{"border-success":this.success,"border-failure":this.failure}},subtext_classes(){return{subtext:!0,"text-success":this.success,"text-failure":this.failure}},subtext_value(){return"string"==typeof this.success?this.success:"string"==typeof this.failure?this.failure:this.subtext},disabled_classes(){return{disabled:this.disabled}}},watch:{value:function(e){this.editor.content.innerHTML!==e&&(this.editor.content.innerHTML=e)}},mounted(){this.editor=pell.init({element:this.$refs.editor,actions:this.actions,defaultParagraphSeparator:this.default_paragraph_separator,styleWithCSS:this.style_with_css,classes:this.classes,onChange:e=>{this.$emit("input",e)}}),this.value&&(this.editor.content.innerHTML=this.value)}});

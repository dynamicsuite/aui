...
Vue.component("aui-button-drop",{template:'<div class="aui btn-drop" ref="dropdown"><button class="aui btn" :class="button_classes" @click="toggle"><span v-if="$slots.default"><slot></slot></span><i :class="icon_class"></i></button><ul v-if="show_dropdown" class="dropdown" :class="menu_align"><li v-for="entry in dropdown" @click="runAction(entry.action)">{{entry.label}}</li></ul></div>',props:{type:{type:String,default:"primary",validator:t=>-1!==["primary","secondary","success","warning","failure"].indexOf(t)},dropdown:{type:Array,required:!0,validator(t){if(!Array.isArray(t))return!1;for(let o=0;o<t.length;o++){if("string"!=typeof t[o].label)return!1;if("function"!=typeof t[o].action&&"string"!=typeof t[o].action)return!1}return!0}},icon_inactive:{type:String,default:"fas fa-caret-down"},icon_active:{type:String,default:"fas fa-caret-up"},menu_align:{style:String,default:"left",validator:t=>-1!==["left","right"].indexOf(t)}},computed:{button_classes(){let t={active:this.show_dropdown};return t["btn-"+this.type]=!0,t},icon_class(){return this.show_dropdown?this.icon_active:this.icon_inactive}},data:()=>({show_dropdown:!1}),methods:{toggle(){this.show_dropdown=!this.show_dropdown},reset(t){this.$refs.dropdown.contains(t.target)||(this.show_dropdown=!1)},runAction(t){"function"==typeof t?t():window.open(t),this.show_dropdown=!1}},created(){window.addEventListener("click",this.reset)},beforeDestroy(){window.addEventListener("click",this.reset)}});
Vue.component("aui-card",{template:'<div class="aui card"><h1>{{header}}</h1><div class="body"><h2>{{title}}</h2><p class="card-text"><slot></slot></p></div></div>',props:{type:{type:String,default:"primary",validator:e=>-1!==["neutral","primary","secondary","success","warning","failure"].indexOf(e)},header:{type:String,required:!0},title:{type:String}}});
Vue.component("aui-checkbox",{template:'<label class="aui checkbox"><slot></slot><input type="checkbox" :checked="checked" :disabled="disabled" @change="$emit(\'input\', $event.target.checked)"><span class="checkmark"></span></label>',props:{checked:{type:Boolean},disabled:{type:Boolean,default:!1}}});
Vue.component("aui-datalist",{template:'<div :id="id + \'-container\'" class="aui datalist-container"><label :for="id" v-if="title">{{title}}</label><input :id="id" :name="name" :type="type" :placeholder="placeholder" :disabled="disabled" :readonly="readonly" :value="value" :class="classes + classFailure() + classSuccess()" :list="id + \'-datalist\'" :failure="failure" :success="success" @keydown="$emit(\'keydown\')" @focus="$emit(\'focus\')" @blur="$emit(\'blur\')" @input="$emit(\'input\', $event.target.value)"><div class="aui subtext" :class="subtextColorClass()" v-if="subtext">{{subtext}}</div><datalist :id="id + \'-datalist\'"><option v-for="option in current_options">{{option}}</option></datalist></div>',props:{id:{type:String,required:!0},options:{type:Array|Object,required:!0},validate:{type:Boolean,default:!0},title:{type:String},classes:{type:String,default:""},name:{type:String},type:{type:String,default:"text"},placeholder:{type:String},disabled:{type:Boolean},readonly:{type:Boolean},success:{default:!1},failure:{default:!1},subtext:{type:String},value:{type:String|Number}},data:function(){return{show_menu:!1,current_options:this.options,search_term:""}},watch:{search_term(){this.searchAndUpdate()},options(){this.searchAndUpdate()}},methods:{searchAndUpdate(){let t=this.search_term.toLowerCase();Array.isArray(this.options)?(this.current_options=this.options.filter(e=>{if(e.toString().toLowerCase().includes(t))return!0}),""===t&&(this.current_options=this.options)):(this.current_options=Object.keys(this.options).filter(e=>{if(this.options[e].toString().toLowerCase().includes(t))return!0}).reduce((t,e)=>(t[e]=this.options[e],t),{}),""===t&&(this.current_options=this.options)),0===Object.keys(this.current_options).length&&(this.current_options=["No options available"])},classSuccess(){return this.success?" border-success":""},classFailure(){return this.failure?" border-failure":""},subtextColorClass(){return this.success?"text-success":this.failure?"text-failure":""}}});
Vue.component("aui-list-group",{template:'<ul class="aui list-group"><aui-list-item v-for="(item, index) in data" :key="index" :callback="item.callback" :subtext="item.subtext">{{item.content}}</aui-list-item></ul>',props:{data:{type:Array,required:!0}}});
Vue.component("aui-list-item",{template:'<li class="aui list-item" :class="activeClass" @click="runCallback()"><span class="text"><div class="title"><slot></slot></div><div class="aui subtext" v-if="subtext">{{subtext}}</div></span><span class="icon" v-if="isActive()"></span></li>',props:{callback:{type:Function},subtext:{type:String}},methods:{isActive(){return!!this.callback},runCallback(){this.callback&&this.callback()}},computed:{activeClass(){return this.callback?" active":""}}});
Vue.component("aui-modal",{template:'<div class="aui modal-container" v-if="state" @click.self="toggleEmit()"><div class="modal"><div v-if="title" class="modal-header" :class="title_classes"><h4>{{title}}</h4><i class="fa fa-times" @click.self="toggleEmit()" v-if="close"></i></div><div class="modal-body"><slot></slot></div></div></div>',props:{state:{type:Boolean,required:!0},title:{type:String},html:{type:String},close:{type:Boolean,default:!0},title_classes:{type:String}},methods:{escEmit(e){27===e.keyCode&&this.$emit("modal-close")},toggleEmit(){this.close&&this.$emit("modal-toggle")}},watch:{state(){this.state&&document.getElementById("ds-nav-container")&&document.getElementById("ds-view-header")?(document.getElementById("ds-nav-container").style.zIndex="0",document.getElementById("ds-view-header").style.zIndex="-1",document.getElementById("ds-nav-container").classList.remove("ds-nav-show-mobile")):!this.state&&document.getElementById("ds-nav-container")&&document.getElementById("ds-view-header")&&(document.getElementById("ds-nav-container").style.zIndex="1",document.getElementById("ds-view-header").style.zIndex="0")}},created:function(){this.close&&document.addEventListener("keyup",this.escEmit)},destroyed:function(){this.close&&document.removeEventListener("keyup",this.escEmit)}});

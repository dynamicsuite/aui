...
Vue.component("aui-currency",{template:'<span class="aui number" :class="negative_class">{{pretty_value}}</span>',props:{is_currency:{type:Boolean,default:!1},currency_sign:{type:String,default:"$"},decimals:{type:Number,default:0,validator:function(e){return e>=0&&parseInt(e)===e}}},computed:{negative_class(){return this.$slots.default[0].text>=0?"":"negative"},pretty_value(){let e=t<0,t=this.$slots.default[0].text,a=this.is_currency?this.currency_sign:"";return t=parseFloat(t.toString().replace(",","")),t=Math.abs(t),(e?`-${a}`:`${a}`)+t.toFixed(2).replace(/\B(?=(\d{3})+(?!\d))/g,",")}}});

...
Vue.component("aui-phone",{template:'<span class="aui phone">{{formatted_phone}}</span>',props:{number:{type:null|String,default:null}},computed:{formatted_phone(){const t=this.number.replace(/[^0-9]/g,""),e=t.length;return 10===e?`(${t.substr(0,3)}) ${t.substr(4,3)}-${t.substr(7,4)}`:11===e&&"1"===t.charAt(0)?`+1 (${t.substr(0,3)}) ${t.substr(4,3)}-${t.substr(7,4)}`:this.number}}});

...
Vue.component("aui-wysiwyg",{template:'<div :id="id + \'-container\'" class="aui wysiwyg"><label>{{label}}</label><div :id="id"></div></div>',props:{id:{type:String,required:!0},label:{type:String},text:{default:""}},data:()=>({editor:null}),mounted(){this.editor=pell.init({element:document.getElementById(this.id),defaultParagraphSeparator:"p",styleWithCSS:!0,onChange:t=>{this.$emit("input",t)},actions:["bold","italic","underline","heading1","ulist","olist"],classes:{actionbar:"pell-actionbar",button:"pell-btn",content:"pell-content",selected:"pell-button-selected"}}),this.editor.content.innerHTML=this.text},watch:{text(){""===this.editor.content.innerHTML&&(this.editor.content.innerHTML=this.text)}}});

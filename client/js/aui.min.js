...
Vue.component("aui-badge",{template:'<span class="aui badge" :class="style_class"><slot>{{text}}</slot></span>',props:{type:{type:String,default:"primary",validator:e=>-1!==["none","primary","secondary","success","warning","failure"].indexOf(e)},text:{type:String|Number}},computed:{style_class(){return{[`badge-${this.type}`]:"none"!==this.type}}}});
...
Vue.component("aui-range",{template:'<aui-form-control v-bind="properties"><template v-slot:label><label><span>{{prefix}}{{lowest}}{{suffix}}</span><span>{{prefix}}{{highest}}{{suffix}}</span></label></template><div class="range-container"><div class="bar"></div><div class="filler" ref="filler"></div><input type="range" ref="slider1" :min="min" :max="max" @mousemove="moveHandler" @input="handleChange" v-model="start" /><input type="range" ref="slider2" :min="min" :max="max" @mousemove="moveHandler" @input="handleChange" v-model="end" /></div></aui-form-control>',props:{min:{type:Number,required:!0},max:{type:Number,required:!0},prefix:{type:null|String,default:null},suffix:{type:null|String,default:null},disabled:{type:Boolean,default:!1},tabindex:{type:String|Number|null,default:null},success:{type:null|String,default:null},failure:{type:null|String,default:null},subtext:{type:null|String,default:null}},data:()=>({start:0,end:0,highest:null,lowest:null}),computed:{properties(){return{parent:"range",label:this.$slots.default?this.$slots.default:this.label,success:this.success,failure:this.failure,subtext:this.subtext}}},methods:{handleChange(){this.$emit("update:min",this.lowest),this.$emit("update:max",this.highest)},moveHandler(e){let t=this.$refs.slider1.getBoundingClientRect().right,s=e.clientX/t,i=Math.floor(Math.abs(this.start-Math.floor(s*this.max))),l=Math.floor(Math.abs(this.end-Math.floor(s*this.max)));e.buttons||(i<l?(this.$refs.slider1.style.zIndex="2",this.$refs.slider2.style.zIndex="1"):(this.$refs.slider2.style.zIndex="2",this.$refs.slider1.style.zIndex="1"))},setHighLow(){this.lowest=Math.min(this.start,this.end),this.highest=Math.max(this.start,this.end)}},watch:{start(){this.setHighLow()},end(){this.setHighLow()}},mounted(){this.start=this.min,this.end=this.max,this.setHighLow()}});
...
Vue.component("aui-select-assignment",{template:'<div class="aui select-assignment"><div class="aui select-assigned"><aui-select :label="assigned_label" :options="assigned" multiple :size="size" :disabled="disabled" v-model="assigned_selected" /><aui-button :text="\'Move to &quot;\' + unassigned_label + \'&quot; list\'" type="secondary" :disabled="disabled" @click="move(\'assigned\')" /></div><div class="aui select-unassigned"><aui-select :label="unassigned_label" :options="unassigned" multiple :size="size" :disabled="disabled" v-model="unassigned_selected" /><aui-button :text="\'Move to &quot;\' + assigned_label + \'&quot; list\'" type="secondary" :disabled="disabled" @click="move(\'unassigned\')" /></div></div>',props:{assigned:{type:Object|Array|null,required:!0},assigned_label:{type:String,default:"Assigned"},unassigned:{type:null|Object,required:!0},unassigned_label:{type:String,default:"Unassigned"},disabled:{type:Boolean,default:!1},size:{type:String|Number|null,default:5}},data:()=>({assigned_selected:[],unassigned_selected:[]}),methods:{move(e){const s=Object.assign({},this.assigned),i=Object.assign({},this.unassigned);for(const t of this[`${e}_selected`])"assigned"===e?(i[t]=s[t],delete s[t]):(s[t]=i[t],delete i[t]);this.$emit("update:assigned",s),this.$emit("update:unassigned",i),this.$emit("update",{assigned:s,unassigned:i}),this[`${e}_selected`]=[]}}});
Vue.component("aui-slider",{template:'<aui-form-control v-bind="properties"><input type="range" :min="min" :max="max" ref="input" :value="value" :tabindex="tabindex" :disabled="disabled" @focus="$emit(\'focus\', $event.target)" @blur="$emit(\'blur\', $event.target)" @change="$emit(\'change\', $event.target.value)" @input="$emit(\'input\', $event.target.value)" @keydown="$emit(\'keydown\', $event)"></aui-form-control>',props:{min:{type:Number,required:!0},max:{type:Number,required:!0},prefix:{type:null|String,default:null},label:{type:null|String,default:null},value:{type:null|Number,default:null},disabled:{type:Boolean,default:!1},tabindex:{type:String|Number|null,default:null},success:{type:null|String,default:null},failure:{type:null|String,default:null},subtext:{type:null|String,default:null}},computed:{properties(){return{parent:"slider",label:this.$slots.default?this.$slots.default:this.label,success:this.success,failure:this.failure,subtext:this.subtext}}}});
